...
Full runs use **SerpApi's `google_scholar_author` endpoint**: parsed JSON with the citation table, the per-year graph, affiliation, and verified-email domain, one call per person, from anywhere including GitHub Actions. Pricing at the time of writing: 250 lookups a month free, $25 for 1,000, capped at 200 searches an hour, so a full run takes about four hours. One paid month per quarter covers a full run of 806 plus the discovery pass, so the annual cost is about $100, or nothing if a run is spread over the free tier across a quarter.
...
**Scholar profile discovery.** For each person without a Scholar id, one call to Scholar's author search through SerpApi returns candidate profiles with affiliation text, verified-email domain, and citation totals (reached directly, that search now demands a Google sign-in; a publication-search fallback from a UT connection exists for spot checks). The same pass re-finds people whose recorded Scholar id has stopped resolving, which happens when Google renumbers a profile. They are scored the same way (name, affiliation, plausibility against the number we already hold) and land in the same review queue. Run once over the 243, then each semester for new hires.
